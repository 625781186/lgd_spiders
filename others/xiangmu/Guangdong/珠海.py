...
class ZhuhaiSpider(TongyongSpider):
    url = 'http://fdcjg.zhzgj.gov.cn/presalelist?keywords=presale&tabkey=all&searchcode=&start=1&count=30'
    ...
    def parse_detail(self, url):
        for i in range(1, 5):
            try:
                ...
                options = html.xpath('//select[@id="buildnum"]/option')
                for option in options:
                    position = option.xpath('./text()')[0]
